...
IsoBmffProcessor::IsoBmffProcessor(class PrivateInstanceAAMP *aamp, id3_callback_t id3_hdl, IsoBmffProcessorType trackType, bool passThrough,
	...
	processPTSComplete(false), timeScale(0),
									...
									bool isInit, process_fcn_t processor, bool &ptsError)
{
	...
	if (!initSegmentProcessComplete)
	{
		if (passThroughMode)
		{
			...
			ret = updatePTSAndTimeScaleFromBuffer(buffer);
		}
		else
		{
			ret = setTuneTimePTS(buffer, position, duration, discontinuous, isInit);
		}
	}
	if (ret)
	{
 ...
 */
bool IsoBmffProcessor::setTuneTimePTS(std::vector<uint8_t>& buffer, double position, double duration, bool discontinuous, bool isInit)
{
	bool ret = true;
		...
		if (ret && !initSegmentProcessComplete)
		{
			if (processPTSComplete)
			{
				...
				else
				{
					...
					sendStream(buffer, pos, duration, 0.0, discontinuous, isInit);
					ret = false;
				}
 ...
 */
void IsoBmffProcessor::restampPTSAndSendSegment(std::vector<uint8_t>& buffer, double position, double duration, bool isDiscontinuity, bool isInit)
{
	uint32_t tScale = 0;
	bool ret = true;
	IsoBmffBuffer bmffBuf;
		...
		*/
		...
		if (bmffBuf.getTimeScale(tScale))
		{
			...
			if(sumPTS == 0 && timeScaleChangeState == eBMFFPROCESSOR_INIT_TIMESCALE)
			{
				...
				currTimeScale = timeScale;
				p_aamp->ProcessID3Metadata(buffer, (AampMediaType)type);
				sendStream(buffer, position, duration, 0.0, isDiscontinuity, isInit);
			}
			...
			else if ( currTimeScale == tScale && sumPTS != 0 && isDiscontinuity == false )
			{
				...
				{
					clearRestampInitSegment();
					cacheRestampInitSegment((AampMediaType)type, buffer, position, duration, isDiscontinuity);
					/*
			...
			else if( currTimeScale != tScale && sumPTS != 0 && isDiscontinuity == false )
			{
				...
				cacheInitBufferForRestampingPTS(buffer, tScale, position, true); // timescale changed with abr scale the pts to continue push
			}
			else
			{
				//time scale is changed save the init buffer for new time scale*/
				cacheInitBufferForRestampingPTS(buffer, tScale, position);
			}
		}
	...
	else
	{
		...
		*/
		...
		Box *pBox =  bmffBuf.getBox(Box::MOOF, index);
		if (NULL != pBox)
		{
			bmffBuf.getSampleDuration(pBox,durationFromFragment);
		...
		uint64_t currentPTS = 0;
		if (bmffBuf.getFirstPTS(currentPTS))
		{
		...
		*/
		...
		else
		{
			...
			bmffBuf.restampPTS(sumPTS, currentPTS, buffer.data(), static_cast<uint32_t>(buffer.size()));
			...
			p_aamp->ProcessID3Metadata(buffer, (AampMediaType)type);
			sendStream(buffer, newPos, duration, 0.0, isDiscontinuity, isInit);
		}
		prevPosition = position;
 ...
 */
void IsoBmffProcessor::cacheInitBufferForRestampingPTS(const std::vector<uint8_t>& segment, uint32_t tScale, double position, bool isAbrChangedTimeScale)
{
 ...
 */
void IsoBmffProcessor::cacheRestampInitSegment(AampMediaType type, const std::vector<uint8_t>& segment, double pos, double duration, bool isDiscontinuity)
{
	std::lock_guard<std::mutex> lock(initSegmentTransferMutex);
	resetPTSInitSegment.push_back({type, segment, pos, duration, isDiscontinuity});
}

/**
 *  @brief Cache init fragment internally
 */
void IsoBmffProcessor::cacheInitSegment(const std::vector<uint8_t>& segment)
{
	std::lock_guard<std::mutex> lock(initSegmentTransferMutex);
...
{
	std::lock_guard<std::mutex> lock(initSegmentTransferMutex);
	if (!resetPTSInitSegment.empty())
	{
		for (auto& entry : resetPTSInitSegment)
		{
			sendStream(entry.buffer, entry.position, entry.duration, 0.0, entry.isDiscontinuity, true);
		}
		resetPTSInitSegment.clear();
	}
	else
	{
...
bool IsoBmffProcessor::updatePTSAndTimeScaleFromBuffer(std::vector<uint8_t>& buffer)
{
	...
	{
		AAMPLOG_WARN("IsoBmffProcessor %s readPTSAndTimeScaleFromBuffer: Buffer is empty", IsoBmffProcessorTypeName[type]);
	}
	return ret;
}
