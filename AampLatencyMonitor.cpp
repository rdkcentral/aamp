...
void AampLatencyMonitor::EnableRateCorrection(bool enabled)
{
	...
	AAMPLOG_INFO("[LatencyMonitor] rate correction %s",
...
void AampLatencyMonitor::Run()
{
	...
	while (mState == State::kRunning)
	{
		...
		// ------------------------------------------------------------------
		// Gate checks — skip the iteration without applying any change.
		// ------------------------------------------------------------------
		if (!mAamp->DownloadsAreEnabled())
		{
			AAMPLOG_WARN("[LatencyMonitor] downloads disabled — exiting loop");
			break;
		}
		...
		{
			AAMPLOG_TRACE("[LatencyMonitor] state=%d, skipping poll", playerState);
			continue;
		}

		// ------------------------------------------------------------------
		// Goal 3: suppress correction during ad playback or explicit disable.
		// ------------------------------------------------------------------
		const bool corrEnabled = mCorrectionEnabled.load();
		const bool adPlaying   = false; //mAamp->IsAdPlaying();

		if (!corrEnabled || adPlaying)
		{
			AAMPLOG_DEBUG("[LatencyMonitor] correction suppressed "
				"(enabled=%d adPlaying=%d)", corrEnabled, adPlaying);
			ResetToNormalRate();
			continue;
		}
