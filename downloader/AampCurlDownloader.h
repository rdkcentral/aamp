...
typedef struct _downloadConfig
{
	...
	_downloadConfig() : pCurl(nullptr),iDownloadTimeout(DEFAULT_CURL_TIMEOUT),iLowBWTimeout(0),iCurlConnectionTimeout(DEFAULT_CURL_CONNECTTIMEOUT),
			...
			iDownloadRetryWaitMs(50),iDownload502RetryWaitMs(MIN_DELAY_BETWEEN_MANIFEST_UPDATE_FOR_502_MS),iDnsCacheTimeOut(DEFAULT_DNS_CACHE_TIMEOUT), bCurlThroughput(false), networkPersonaFile(), bEnableHTTP3(false)
	{
	}
