...
def robust_std(x):
    # robust stdev via IQR -> approximate sigma ~= IQR/1.349 for normal-like tails
...
def fit_bursts(bur_csv, guard_low=0.10, guard_high=0.50):
    ...
    cadence_jitter_ms = float(gaps[mask_cadence].std(ddof=1) * 1000.0) if mask_cadence.any() else float(gaps.std(ddof=1) * 1000.0)
