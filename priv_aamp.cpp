...
void PrivateInstanceAAMP::_ReportProgress(bool sync, bool beginningOfStream)
{
		...
		(state != eSTATE_SEEKING))
	{
		...
		**  -A good candidate for future refactoring*/
		...
		ProgressEventPtr evt = std::make_shared<ProgressEvent>(duration, reportFormattedCurrPos, start, end, speed, videoPTS, bufferedDuration, seiTimecode.c_str(), latency, bps, mNetworkBandwidth, currentRate, _GetSessionId());
 ...
 *
 */
void PrivateInstanceAAMP::_Stop()
{
 ...
 */
void PrivateInstanceAAMP::_SendMonitorAvEvent(const std::string &status, int64_t videoPositionMS, int64_t audioPositionMS, uint64_t timeInStateMS)
{
	if(mbPlayEnabled)
	{
		MonitorAVStatusEventPtr evt = std::make_shared<MonitorAVStatusEvent>(status, videoPositionMS, audioPositionMS, timeInStateMS, _GetSessionId());
		mEventManager->SendEvent(evt, AAMP_EVENT_SYNC_MODE);
	}
}
/**
