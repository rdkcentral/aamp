				...
				context->mediaType ==  eMEDIATYPE_SUBTITLE))
			{
				...
				if (context->chunkBoundary > 0)
				{
					if (context->buffer->GetLen() >= context->chunkBoundary)
					{
						...
						size_t bufferLen = 0;
						AAMPLOG_WARN("[%d] Reached chunk boundary at start %zu size %zu (current buffer len %zu)", context->mediaType, context->bufferOffset, context->chunkBoundary, context->buffer->GetLen());
