...
{
	bool abort = false;
		// BEGIN SIMULATION: one-shot early abort on first video segment
	if (context->mediaType == eMEDIATYPE_VIDEO)
	{
		static bool sAbortInjected = false;
		static std::string lastUrl;
		static int cnt = 1;
		if (!sAbortInjected && lastUrl != context->remoteUrl && context->chunkedDownload)
		{
			lastUrl = context->remoteUrl;
			cnt++;
			if (cnt % 10 == 0) // inject abort every 10th video segment to avoid excessive aborts during testing
			{
				sAbortInjected = true;
				AAMPLOG_WARN("SIMULATED early abort for video segment (slow download)");
