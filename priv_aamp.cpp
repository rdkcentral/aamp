...
//	SETCONFIGVALUE_PRIV(AAMP_STREAM_SETTING, eAAMPConfig_InterruptHandling, (mFogTSBEnabled && strcasestr(mainManifestUrl, "networkInterruption=true")));
...

	{
		...
		AAMPLOG_MIL("%s Debug--> aamp_tune: attempt: %d format: %s URL: %s", tuneStrPrefix, mTuneAttempts, mMediaFormatName[mMediaFormat], mManifestUrl.c_str());
		if(!mMPDStichRefreshUrl.empty())
		{
			AAMPLOG_WARN("%s DEBUG aamp_stich: Option[%d] URL: %s", tuneStrPrefix, mMPDStichOption, mMPDStichRefreshUrl.c_str());
		}
		if(IsFogTSBSupported())
		{
