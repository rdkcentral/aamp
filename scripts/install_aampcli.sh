...
function aampcli_install_build_darwin_fn()
{
    ...
    cd build && PKG_CONFIG_PATH=${PKG_CONFIG}:${PKG_CONFIG_PATH} cmake -DDISTRO=ON -DCMAKE_BUILD_TYPE=Debug -DCOVERAGE_ENABLED=${OPTION_COVERAGE} -DUTEST_ENABLED=ON -DCMAKE_INBUILT_AAMP_DEPENDENCIES=1 -DCMAKE_ENABLE_PTS_RESTAMP:BOOL=TRUE -G Xcode ../
