...
void AampStreamSinkManager::DeleteStreamSink(PrivateInstanceAAMP *aamp)
{
	...
	switch(mPipelineMode)
	{
		...
		case ePIPELINEMODE_MULTI:
		{
			AampStreamSinkInactive* inactiveSink = mInactiveGstPlayersMap[aamp];
			if (inactiveSink)
			{
				mInactiveGstPlayersMap.erase(aamp);
				delete(inactiveSink);
			...
			AAMPGstPlayer* activeSink = mActiveGstPlayersMap[aamp];
			if (activeSink)
			{
				mActiveGstPlayersMap.erase(aamp);
				delete(activeSink);
	...
	switch(mPipelineMode)
	{
		case ePIPELINEMODE_SINGLE:
		{
			...
			if (mActiveGstPlayersMap.size() == 0)
			{
				...
				{
					AAMPLOG_ERR("AampStreamSinkManager(%p)::%s Single Pipeline mode, mGstPlayer is null, can's set active PLAYER[%d]", this, __FUNCTION__, aamp->mPlayerId);
				}
			}
		}
		break;
...
StreamSink* AampStreamSinkManager::GetStreamSink(PrivateInstanceAAMP *aamp)
{
	...
	if (mActiveGstPlayersMap.count(aamp) != 0)
	{
	...
	{
		AAMPLOG_TRACE("AampStreamSinkManager(%p)::%s Returning inactive Stream Sink foundf or PLAYER[%d]", this, __FUNCTION__, aamp->mPlayerId);
