...
PersistentWatermark::Storage::Storage():mMetaData(""), mSharedMemoryKey(0), msize(0), mMutex()
{
	std::srand(std::time(nullptr));
}

void PersistentWatermark::Storage::deleteSharedMemory()
{
	LOG_TRACE("Enter");
	if(mSharedMemoryKey)
	{
...
bool PersistentWatermark::Storage::valid()
{
	std::lock_guard<std::mutex>lock(mMutex);
	return msize>PNG_MIN_SIZE;
}
...
int PersistentWatermark::Storage::getKey()
{
	std::lock_guard<std::mutex>lock(mMutex);
	return mSharedMemoryKey;
}

int PersistentWatermark::Storage::getSize()
{
	std::lock_guard<std::mutex>lock(mMutex);
	return msize;
}
