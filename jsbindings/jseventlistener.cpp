	...
	, p_jsCallback(jsCallback)
{
	if (p_jsCallback != NULL)
	{
	 ...
	 LOG_INFO("Sling:: Created AAMP_JSEventListener: %p for eventType: %d, jsCallback: %p", this, type, jsCallback);
}
...
AAMP_JSEventListener::~AAMP_JSEventListener()
{
	...
	LOG_INFO("Sling:: Deleted AAMP_JSEventListener: %p for eventType: %d, jsCallback: %p", this, p_type, p_jsCallback);
}
...
void AAMP_JSEventListener::Event(const AAMPEventPtr& e)
{
	...
	LOG_INFO("Sling:: Dispatching event of type: %d to eventListener: %p, jsCallback: %p", evtType, this, p_jsCallback);
...
void AAMP_JSEventListener::AddEventListener(PrivAAMPStruct_JS* obj, AAMPEventType type, JSObjectRef jsCallback)
{
	LOG_TRACE("(%p, %d, %p)", obj, type, jsCallback);
	 LOG_INFO("Sling:: Adding eventListener for eventType: %d, jsCallback: %p", type, jsCallback);
	...
	LOG_INFO("Sling:: Added eventListener: %p for eventType: %d", pListener, type);
}
...
{
        LOG_TRACE("(%p, %d, %p)", obj, type, jsCallback);
	 LOG_INFO("Sling:: Removing eventListener for eventType: %d, jsCallback: %p", type, jsCallback);
		...
		for(listenerIter_t iter = range.first; iter != range.second; )
		{
			...
			if (listener->p_jsCallback == jsCallback)
			{
				...
				SAFE_DELETE(listener);
				 LOG_INFO("Sling:: Removed eventListener: %p for eventType: %d", listener , type);
			}
			else
			{
				iter++;
	...
	LOG_INFO("Sling:: RemoveEventListener Js Exit");
}
