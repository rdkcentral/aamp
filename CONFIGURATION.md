...
| `synthesizeIframeForVOD` | Boolean | false | When `true`, AAMP uses the regular video track during VOD DASH trickplay (FF/REW) to synthesize I-frame segments. The download is terminated early once the first sample payload has been received (determined by `GetIframeByteCap()`), then `IsoBmffHelper::ConvertToKeyFrame()` strips the MOOF to a single key frame. Enables VCR-style trickplay on VOD assets (including JITT ads) that do not advertise a dedicated iframe `AdaptationSet`. Has no effect during live/TSB playback. |
