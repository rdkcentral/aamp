 ...
 */
static void  registerCb(AampLicenseManager* _this, DRMSessionManager* instance)
{
...
/**
 *  @brief AampLicenseManager constructor.
 */
AampLicenseManager::AampLicenseManager(int maxDrmSessions, PrivateInstanceAAMP *aamp) : mMaxDRMSessions(maxDrmSessions),
...
/**
 *  @brief AampLicenseManager destructor
 */
 ...
 */
void AampLicenseManager::releaseLicenseRenewalThreads()
{
 ...
 */
void AampLicenseManager::setLicenseRequestAbort(bool isAbort)
{
	mAccessTokenConnector.Release();
	licenseRequestAbort = isAbort;
}
void AampLicenseManager::licenseRenewalThread(std::shared_ptr<DrmHelper> drmHelper, int sessionSlot, PrivateInstanceAAMP* aampInstance)
{
...
void AampLicenseManager::renewLicense(std::shared_ptr<DrmHelper> drmHelper, void* userData, PrivateInstanceAAMP* aampInstance)
{
 ...
 */
KeyState AampLicenseManager::acquireLicense(std::shared_ptr<DrmHelper> drmHelper, int sessionSlot, int &cdmError,
...
KeyState AampLicenseManager::handleLicenseResponse(std::shared_ptr<DrmHelper> drmHelper, int sessionSlot, int &cdmError, int32_t httpResponseCode, int32_t httpExtendedStatusCode, shared_ptr<DrmData> licenseResponse, DrmMetaDataEventPtr eventHandle,  bool isLicenseRenewal)
{
...
KeyState AampLicenseManager::processLicenseResponse(std::shared_ptr<DrmHelper> drmHelper, int sessionSlot, int &cdmError,
 ...
 */
void AampLicenseManager::UpdateLicenseMetrics(DrmRequestType requestType, int32_t statusCode, std::string licenseRequestUrl, long long downloadTimeMS, DrmMetaDataEventPtr eventHandle, DownloadResponsePtr respData )
{
 ...
 */
const char * AampLicenseManager::getAccessToken(int &tokenLen, int &error_code , bool bSslPeerVerify)
{	
	if(accessToken == NULL)
	{
 ...
 */
bool AampLicenseManager::configureLicenseServerParameters(std::shared_ptr<DrmHelper> drmHelper, LicenseRequest &licenseRequest,
...
void AampLicenseManager::ContentProtectionDataUpdate(PrivateInstanceAAMP* aampInstance, std::vector<uint8_t> keyId, AampMediaType streamType)
{
 ...
 */
bool AampLicenseManager::QueueContentProtection(std::shared_ptr<DrmHelper> drmHelper, std::string periodId, uint32_t adapIdx, AampMediaType type, bool isVssPeriod)
{
 ...
 */
DrmData * AampLicenseManager::getLicense(LicenseRequest &licenseRequest,
 ...
 */
void AampLicenseManager::ProfilerUpdate()
{

  aampInstance->profiler.ProfileBegin(PROFILE_BUCKET_LA_PREPROC);
}
std::string  AampLicenseManager::HandleContentProtectionData(std::shared_ptr<DrmHelper> drmHelper, int streamType, std::vector<uint8_t> keyId, int contentProtectionUpd)
{
 ...
 * @return none
 */
void AampLicenseManager::QueueProtectionEvent(std::shared_ptr<DrmHelper> drmHelper, std::string periodId, uint32_t adapIdx, AampMediaType type)
{
 ...
 */
void AampLicenseManager::setPlaybackSpeedState(bool live, double currentLatency, bool livepoint , double liveOffsetMs,int speed, double positionMs, bool firstFrameSeen)
{
 ...
 */
void AampLicenseManager::hideWatermarkOnDetach(void)
{
 ...
 */
void AampLicenseManager::UpdateMaxDRMSessions(int maxSessions)
{
 ...
 */
DrmSession* AampLicenseManager::createDrmSession( std::shared_ptr<DrmHelper> drmHelper, DrmCallbacks* aampInstance, DrmMetaDataEventPtr eventHandle, int streamTypeIn)
{
	int err = -1;
 ...
 */
DrmSession * AampLicenseManager::createDrmSession(
