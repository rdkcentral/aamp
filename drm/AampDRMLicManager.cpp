...
void AampDRMLicenseManager::TriggerDecryptProfile(int streamType, int action, int result /* = 0 */)
{
    ...
    switch ((ProfilerAction)action)
    {
        case PROFILE_ACTION_BEGIN:
			AAMPLOG_INFO("Decrypt Profile Begin for streamType %d", streamType);
        ...
        case PROFILE_ACTION_END:
			AAMPLOG_INFO("Decrypt Profile End for streamType %d", streamType);
            ...
            aampInstance->profiler.ProfilePerformed(PROFILE_BUCKET_DECRYPTION_END);
            break;
        case PROFILE_ACTION_ERROR:
			AAMPLOG_INFO("Decrypt Profile Error for streamType %d", streamType);
