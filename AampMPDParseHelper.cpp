		...
		else if(mMPDInstance != NULL )
		{
			if(periodIndex < mNumberOfPeriods)
			{
				...
				if(!startTimeStr.empty())
				{
					...
					AAMPLOG_INFO("StreamAbstractionAAMP_MPD: - MPD periodIndex %d periodId %s AvailStartTime %f periodStart %f %s", periodIndex, mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), mAvailabilityStartTime, periodStart,startTimeStr.c_str());
				}
			...
			if(periodIndex < mNumberOfPeriods)
			{
				...
				if (1 == mNumberOfPeriods && !mIsLiveManifest)
				{
					...
					if(mMediaPresentationDuration != 0 )
					{
						...
						AAMPLOG_MIL("period duration based on mMediaPresentationDuration periodId %s =%f", mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), periodDurationMs );
					...
					else if(!durationStr.empty() )
					{
						...
						AAMPLOG_WARN("period duration based on duration field periodId %s =%f", mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), periodDurationMs );
				...
				if(!durationStr.empty())
				{
					...
					AAMPLOG_INFO("StreamAbstractionAAMP_MPD: MPD periodIndex:%d periodId %s periodDuration %f", periodIndex, mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), periodDuration);
				}
				else
				{
					if(mNumberOfPeriods == 1 && periodIndex == 0)
					{
						...
						periodDuration = (periodDurationMs / 1000.0);
						AAMPLOG_INFO("StreamAbstractionAAMP_MPD: [MediaPresentation] - MPD periodIndex:%d periodId %s periodDuration %f", periodIndex, mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), periodDuration);
					}
					else
					{
						...
						if(!curStartStr.empty() && (!nextStartStr.empty()) && !IsUninterruptedTSB)
						{
							...
							if(periodDuration != 0.0f)
								AAMPLOG_INFO("StreamAbstractionAAMP_MPD: [StartTime based] - MPD periodIndex:%d periodId %s periodDuration %f", periodIndex, mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), periodDuration);
						}
						else
						{
							...
							AAMPLOG_INFO("StreamAbstractionAAMP_MPD: [Segments based] - MPD periodIndex:%d periodId %s periodDuration %f", periodIndex, mMPDInstance->GetPeriods().at(periodIndex)->GetId().c_str(), periodDuration);
						}
					}
				}
			}
