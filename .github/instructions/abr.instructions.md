---
description: ABR and latency control normative specification — rules Copilot must apply when working in the abr/ module
applyTo:
  - "abr/**"
---
...
It is the authoritative reference for correctness when generating, editing, or reviewing code in this module.
...
### 1.4 networkHistory
Historical measurements used to estimate network throughput and request time-to-first-byte.
...
- Bounded: `cfgDefaultTargetLatency ≤ targetLatency ≤ cfgMaxTargetLatency`
...
- No semantic distinction between initial tune and post-underflow recovery
...
A predicted underflow occurs if, given current `buffer` and predicted real-world download time of required next media, the buffer is expected to reach `0` before new decodable media is injected.
...
On transition from `buffer > cfgDangerBuffer` → `buffer ≤ cfgDangerBuffer`:
...
If `stableBufferTime > cfgLatencyStable` and `targetLatency > cfgDefaultTargetLatency`:
