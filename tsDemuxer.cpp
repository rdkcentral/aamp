...
void Demuxer::send()
{
	if (CheckForSteadyState())
	{
		...
		if (aamp)
		{
			aamp->SendStreamCopy(type, es.data(), es.size(), info.pts_s, info.dts_s, duration);
		}
		es.clear();
	}
}
...
void Demuxer::sendInternal(MediaProcessor::process_fcn_t processor)
{
	if (processor)
	{
		if (CheckForSteadyState())
		{
			...
			uint8_t * data_ptr = reinterpret_cast<uint8_t *>(es.data());
		...
		/*PARSE PES*/
		{
			...
			while (size > 0)
			{
				...
				case PES_STATE_GETTING_HEADER:
					...
					pes_header.insert(pes_header.end(), data, data + bytes_to_read);
					data += bytes_to_read;
					size -= bytes_to_read;
				...
				case PES_STATE_GETTING_ES:
					...
					es.insert(es.end(), data, data + size);
					size = 0;
					break;
				default:
					pes_state = PES_STATE_WAITING_FOR_HEADER;
