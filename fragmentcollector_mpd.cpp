...
double StreamAbstractionAAMP_MPD::GetPeriodStartTime(IMPD *mpd, int periodIndex)
{
	...
	{
		int periodCnt= (int)mpd->GetPeriods().size();
		if(periodIndex < periodCnt)
		{
			...
			if(!startTimeStr.empty())
			{
				...
				if(periodCnt == 1 && periodIndex == 0 && aamp->IsLiveStream() && !aamp->IsTSBSupported() && (periodStart == mAvailabilityStartTime) && deltaInStartTime == 0)
				{
					...
					double duration = (aamp_GetPeriodDuration(mpd, periodIndex, mLastPlaylistDownloadTimeMs) / 1000);
	...
	if(mpd != NULL)
	{
		...
		if(periodIndex < periodCnt)
		{
			...
			if(!durationStr.empty())
			{
				periodDurationMs = ParseISO8601Duration(durationStr.c_str());
				double liveTime = (double)mLastPlaylistDownloadTimeMs / 1000.0;
				if(!aamp->IsTSBSupported() && aamp->IsLive() && mLiveTimeFragmentSync && (mAvailabilityStartTime > (liveTime - (periodDurationMs/1000))))
				{
						...
						if(!periodStartStr.empty())
						{
							...
							else if(periodIndex == (periods.size() - 1))
							{
								...
								else
								{
									...
									std::string tsbDepth = mpd->GetTimeShiftBufferDepth();
									AAMPLOG_INFO("periodStart %lf availabilityStartTime %lf minUpdatePeriod %lf mpdDownloadTime %" PRIu64 " tsbDepth:%s", periodStart, availablilityStart, minUpdatePeriod, mpdDownloadTime, tsbDepth.c_str());
									if(periodStartTime == availablilityStart)
									{
										...
										if((mpdDownloadTime - durationMs) < availablilityStart)
										{
											durationMs = mpdDownloadTime - availablilityStart;
										}
									}
									else
									{
	...
	if (ret == eAAMPSTATUS_OK)
	{
		...
		if (mIsLiveStream)
		{
			...
			if (liveAdjust)
			{
				...
				mCurrentPeriodIdx = (int)mpd->GetPeriods().size();
...
void StreamAbstractionAAMP_MPD::IndexNewMPDDocument(bool updateTrackInfo)
{
	...
	mNumberOfPeriods = (int)mpd->GetPeriods().size();
...
AAMPStatusType StreamAbstractionAAMP_MPD::FetchDashManifest()
{
	...
	if (gotManifest)
	{
		...
		ret = GetMpdFromManifest(manifest, mpd, manifestUrl, true);
...
void StreamAbstractionAAMP_MPD::UpdateCulledAndDurationFromPeriodInfo(std::vector<PeriodInfo> &currMPDPeriodDetails)
{
	...
	if(firstPeriod)
	{
		int lastPeriodIdx = (int)mpd->GetPeriods().size() - 1;
		for(int iPeriod = (int)mpd->GetPeriods().size() - 1 ; iPeriod >= 0; iPeriod--)
		{
			IPeriod *period = mpd->GetPeriods().at(iPeriod);
			if(IsEmptyPeriod(period))
			{
			...
			while((mIterPeriodIndex < mNumberOfPeriods) && (mIterPeriodIndex >= 0) && !exitFetchLoop)  //CID:95090 - No effect
			{
				...
				if (periodChanged || mpdChanged || adStateChanged)
				{
					...
					if (periodChanged)
					{
						...
						{
							mBasePeriodOffset += (GetPeriodDuration(mpd, mIterPeriodIndex)/1000.00);	//Already reached -ve. Subtracting from current period duration
						}
						//Update period gaps for playback stats
						if(mIsLiveStream)
						{
							double periodGap = (GetPeriodEndTime(mpd, mCurrentPeriodIdx, mLastPlaylistDownloadTimeMs) -GetPeriodStartTime(mpd, mIterPeriodIndex)) * 1000;
							if(periodGap > 0)
							{
				...
				while (!exitFetchLoop)
				{
					...
					if (vEos || aEos)
					{
						bool eosOutSideAd = (AdState::IN_ADBREAK_AD_PLAYING != mCdaiObject->mAdState &&
								((rate > 0 && mCurrentPeriodIdx >= mUpperBoundaryPeriod) || (rate < 0 && mLowerBoundaryPeriod == mCurrentPeriodIdx)));
					...
					if(cacheFullStatus[eMEDIATYPE_VIDEO] || (vEos && !aEos))
					{
						...
						int trackIdx = vEos == true ? eMEDIATYPE_AUDIO : eMEDIATYPE_VIDEO;
			...
			if(AdEvent::DEFAULT == evt)
			{
				...
				else
				{
					...
					else
					{
						...
						std::string prevPId = "";
						size_t mNumberOfPeriods = mpd->GetPeriods().size();
