				...
				if(period != NULL)
				{
					...
					for (size_t j=0; j < children.size(); j++) {
						...
						if(!name.empty())
						{
							...
							if((name == "EventStream") && ("" != prdId) &&(!mCdaiObject->isPeriodExist(prdId,StartvalueMS)))
							{
								...
								if (processEventsInPeriod)
								{
									...
									mCdaiObject->InsertToPeriodMap(period, StartvalueMS);
									AAMPLOG_INFO("Inserted Period ID: %s with startTimeMS: %" PRIu64 " to Period Map", prdId.c_str(), StartvalueMS);
									ProcessEventStream(StartvalueMS, firstSegmentStartTime, period, reportBulkMeta);
									AAMPLOG_INFO("Completed Processing EventStream for Period ID: %s and PeriodStart %" PRIu64 , prdId.c_str(), StartvalueMS);
