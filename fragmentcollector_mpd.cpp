...
{ // given url, synchronously download and transmit associated fragment
	...
	if(future.valid())
	{
		...
		{
			// If the job is submitted successfully, we can wait for it to complete
			try
			{
				future.get(); // Wait for the job to complete
			}
			catch (const std::exception& e)
			{
				AAMPLOG_ERR("Exception in Job: %s", e.what());
				retval = false;
			}
		}
	}
	...
	{
		AAMPLOG_ERR("Failed to submit download job for fragment: %s", downloadInfo->urlList.begin()->second.url.c_str());
		retval = false;
	}
			...
			while (!exitFetchLoop)
			{
				...
				if (vEos || aEos)
				{
					...
					if (vEos && aEos)
					{
						...
						//			dependency by saving period specifc profile data, this wait can be removed.
