	...
	for (auto &representation : adaptationSet->GetRepresentation())
	{
		...
		if (segmentTemplates.HasSegmentTemplate())
		{
			...
			else
			{
				...
				ConstructFragmentURL(uriInfo.url, fragmentDescriptor.get(), urlTemplate, aamp->mConfig);
		...
		if (auto segmentList = representation->GetSegmentList())
		{
			if (isInit)
			{
				...
				if (!initUrl.empty())
				{
					ConstructFragmentURL(uriInfo.url, fragmentDescriptor.get(), initUrl, aamp->mConfig);
					uriList[fragmentDescriptor->Bandwidth] = uriInfo;
				}
				else
				{
