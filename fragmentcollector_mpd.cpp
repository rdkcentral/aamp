				...
				else
				{
					...
					*/
					if (!skipFetch && ((pMediaStreamContext->fragmentDescriptor.Time > pMediaStreamContext->lastSegmentTime) || (0 == pMediaStreamContext->lastSegmentTime)))
					{
						...
						else if((mIsFogTSB && (ISCONFIGSET(eAAMPConfig_InterruptHandling))) || (!pMediaStreamContext->mCheckForRampdown && pMediaStreamContext->mDownloadedFragment.GetLen() == 0))
						{
...
void StreamAbstractionAAMP_MPD::AdvanceTrack(int trackIdx, bool trickPlay, double &delta)
{
	...
	if (pMediaStreamContext->adaptationSet)
	{
		...
		{
			if (trickPlay && pMediaStreamContext->mDownloadedFragment.GetLen() == 0 && !pMediaStreamContext->freshManifest)
			{
				double skipTime = 0;
				skipTime = delta;
