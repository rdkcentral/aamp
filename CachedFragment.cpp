...
void CachedFragment::Copy(CachedFragment* other, size_t len)
{
	...
	if (other && !other->fragment.empty() && len > 0) {
		this->fragment.insert(this->fragment.end(), other->fragment.begin(), other->fragment.begin() + len);
	}
}
