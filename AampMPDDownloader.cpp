...
AampMPDDownloader::AampMPDDownloader() :  mMPDBufferQ(),mMPDBufferSize(1),mMPDBufferMutex(),mRefreshMtx(),mRefreshCondVar(),
	...
	mManifestRefreshStatus(ManifestRefreshStatus()),
...
void AampMPDDownloader::downloadMPDThread1()
{
	...
	do
	{
		...
		{
			int errorCode = 0;
			if (mMPDData->mMPDStatus == AAMPStatusType::eAAMPSTATUS_MANIFEST_DOWNLOAD_ERROR)
			{
			...
			else
			{
				...
				bool sameAsPreviousFailure =
					(previousStatus.type == retryStatus.type) &&
						(previousStatus.errorCode == retryStatus.errorCode);
				mManifestRefreshRetryFailureCount.store(sameAsPreviousFailure ? mManifestRefreshRetryFailureCount.load() + 1 : 1);
			}
			mManifestRefreshStatus.store(retryStatus);
		}
		...
		// Skipping HTTP 404 as its not likely to recover on retry
		if (!firstDownload &&
			mMPDData->mMPDStatus != AAMPStatusType::eAAMPSTATUS_OK &&
