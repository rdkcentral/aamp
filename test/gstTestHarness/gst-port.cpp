...
static std::queue<SeekParam> mSegmentEndSeekQueue; // TODO: move into pipeline class
...
public:
	...
	void found_source( GObject * object, GObject * orig, GParamSpec * pspec )
	{
		...
		SeekParam param = mSegmentEndSeekQueue.front();
		mSegmentEndSeekQueue.pop();
		Seek( param );
	}
...
{
	return mSegmentEndSeekQueue.size();
}
...
void Pipeline::Seek( const SeekParam &param )
{
	...
	if( param.flags & GST_SEEK_FLAG_FLUSH )
	{
		...
		std::swap( mSegmentEndSeekQueue, empty );
	}
}
