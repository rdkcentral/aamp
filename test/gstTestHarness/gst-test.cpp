	...
	bool Inject( MyPipelineContext *context, MediaType mediaType )
	{
		...
		else if( mp4Demux )
		{
			...
			{ // media segment
				...
				if(random_count == 1)
				{
					//context->pipeline->SendSegmentEndEvent( mediaType, lastPts);
					context->pipeline->SendSegmentEvent( mediaType, 0, lastPts);
					context->pipeline->SendSegmentEvent( mediaType, lastPts);
					random_count++;
				}
			}
			else
