...
void MediaStreamContext::OnFragmentDownloadSucess(DownloadInfoPtr downloadInfo)
{
}

bool MediaStreamContext::DownloadFragment(DownloadInfoPtr downloadInfo)
{
	if(downloadInfo->urlList.size() > 0)
	{
		downloadInfo->url = downloadInfo->urlList.begin()->second.url;
	}
