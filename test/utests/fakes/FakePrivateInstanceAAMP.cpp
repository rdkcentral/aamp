...
void PrivateInstanceAAMP::SendAdResolvedEvent(const std::string &adId, bool status, uint64_t startMS, uint64_t durationMs,uint64_t mErrorCode, const std::string &mErrorDescription)
{
    if (g_mockPrivateInstanceAAMP != nullptr)
    {
        g_mockPrivateInstanceAAMP->SendAdResolvedEvent(adId, status, startMS, durationMs, mErrorCode, mErrorDescription);
    }
}
