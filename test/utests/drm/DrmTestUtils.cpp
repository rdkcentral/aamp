...
TestUtilDrm::TestUtilDrm(PrivateInstanceAAMP* privAamp)
	: mAamp(privAamp)
{
	mLicenseManager = aamp_utils::make_unique<AampLicenseManager>(2, mAamp);
}
...
AampLicenseManager* TestUtilDrm::getSessionManager()
{
...
{
	AampLicenseManager* sessionManager = getSessionManager();
