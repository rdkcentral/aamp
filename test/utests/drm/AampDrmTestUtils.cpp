...
#ifdef USE_OPENCDM
AampDrmSession* TestUtilDrm::createDrmSessionForHelper(std::shared_ptr<AampDrmHelper> drmHelper,
...
AampDrmSession* TestUtilDrm::createDashDrmSession(const std::string testKeyData,
...
AampDrmSession* TestUtilDrm::createDashDrmSession(const std::vector<uint8_t> testKeyData,
												  const std::string psshStr,
												  DrmMetaDataEventPtr& event)
{
	AampDRMSessionManager* sessionManager = getSessionManager();
