...
TEST_F(IsoBmffHelperTests, restampPtsTest)
{
	...
	// Set the pointer and length in the std::vector<uint8_t> fake
	buffer.insert(buffer.end(), reinterpret_cast<const uint8_t*>(bufferContent), reinterpret_cast<const uint8_t*>(bufferContent) + sizeof(bufferContent));
	...
	auto expectedPtr = reinterpret_cast<uint8_t*>(buffer.data());
