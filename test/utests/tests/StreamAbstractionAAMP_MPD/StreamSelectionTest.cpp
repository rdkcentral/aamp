...
// Minimal VOD manifest containing a DASHIF trickmode iframe adaptation set.
// Used by StreamSelection_TrickPlay_NullAudioContext_DoesNotCrash to exercise
// the trick-play code path (mMaxTracks==1) where only the video context is
// allocated and mMediaStreamContext[eMEDIATYPE_AUDIO] is legitimately null.
static constexpr const char *mIframeManifest = R"(<?xml version="1.0" encoding="utf-8"?>
<MPD xmlns="urn:mpeg:dash:schema:mpd:2011" type="static" mediaPresentationDuration="PT30S" minBufferTime="PT4S" profiles="urn:mpeg:dash:profile:isoff-live:2011">
	<Period id="p0" start="PT0S">
		<AdaptationSet id="0" contentType="video" mimeType="video/mp4">
			...
			<SegmentTemplate timescale="2500" initialization="iframe_init.mp4" media="iframe_$Number$.m4s" startNumber="1">
				<SegmentTimeline>
					<S t="0" d="5000" r="14" />
				</SegmentTimeline>
			</SegmentTemplate>
			<Representation id="iframe0" bandwidth="200000" codecs="avc1.640028" width="1280" height="720" frameRate="1"/>
		</AdaptationSet>
	</Period>
</MPD>)";

/**
 * @brief Regression test for VPAAMP-387: StreamSelection must not dereference
 *        mMediaStreamContext[eMEDIATYPE_AUDIO] when it is null.
 *
 * At trick-play rate (rate != AAMP_NORMAL_PLAY_RATE) Init() sets mMaxTracks=1
 * and allocates only mMediaStreamContext[eMEDIATYPE_VIDEO]; the audio context
 * stays nullptr.  StreamSelection() must complete without a crash, the null
 * guard on the audio-only copy block and the targeted null guard inside the
 * SetCurrentBandWidth loop must both be exercised silently (no AAMPLOG_ERR,
 * since a null audio context is expected at trick-play rate).
 */
TEST_F(StreamSelectionTests, StreamSelection_TrickPlay_NullAudioContext_DoesNotCrash)
{
	EXPECT_CALL(*g_mockMediaStreamContext, CacheFragment(_, _, _, _, _, _, _, _, _))
		.Times(AnyNumber())
		.WillRepeatedly(Return(true));

	// Initialize at trick-play rate; mMaxTracks is set to 1 so only the video
	// context (index 0) is allocated.  The audio context (index 1) is left null.
	constexpr float trickPlayRate = 4.0f;
	AAMPStatusType status = InitializeMPD(mIframeManifest, TuneType::eTUNETYPE_NEW_NORMAL, 0.0, trickPlayRate);
	EXPECT_EQ(status, eAAMPSTATUS_OK);

	// Confirm the precondition: audio context must be null at trick-play rate.
	EXPECT_EQ(mStreamAbstractionAAMP_MPD->GetMediaStreamContext(eMEDIATYPE_AUDIO), nullptr);

	// Video context must have been allocated.
	EXPECT_NE(mStreamAbstractionAAMP_MPD->GetMediaStreamContext(eMEDIATYPE_VIDEO), nullptr);
	...
	// Video context must still be valid after StreamSelection() returns.
	EXPECT_NE(mStreamAbstractionAAMP_MPD->GetMediaStreamContext(eMEDIATYPE_VIDEO), nullptr);
}