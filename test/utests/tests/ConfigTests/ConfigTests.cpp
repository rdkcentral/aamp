...
/**
 * @brief Test eAAMPConfig_TLSVersion default configuration
 * 
 * Verifies that with default configuration, TLS version is set to CURL_SSLVERSION_TLSv1_2 (value 6).
 * This is the minimum required version, allowing libcurl to negotiate the best version supported
 * by both client and server, typically resulting in TLS1.3 on RDK devices.
 */
TEST_F(AampConfigTests, TLSVersionDefault)
{
	mAampConfig->Initialize();
	
	// Default should be CURL_SSLVERSION_TLSv1_2 (value 6)
	// This allows negotiation up to TLS1.3
	int tlsVersion = mAampConfig->GetConfigValue(eAAMPConfig_TLSVersion);
	EXPECT_EQ(tlsVersion, 6) << "Default TLS version should be CURL_SSLVERSION_TLSv1_2 (6)";
	
	// Verify config owner is default
	EXPECT_EQ(mAampConfig->GetConfigOwner(eAAMPConfig_TLSVersion), AAMP_DEFAULT_SETTING);
}

/**
 * @brief Test eAAMPConfig_TLSVersion with supportTLS=393216
 * 
 * Verifies that setting supportTLS=393216 (CURL_SSLVERSION_MAX_TLSv1_2) forces the maximum
 * TLS version to TLSv1.2. This value is (6 << 16) which sets both minimum and maximum to TLS1.2.
 * 
 * CURL_SSLVERSION_MAX_TLSv1_2 = 0x60000 = 393216
 */
TEST_F(AampConfigTests, TLSVersionMaxTLS12)
{
	mAampConfig->Initialize();
	...
	EXPECT_EQ(tlsVersion, CURL_SSLVERSION_MAX_TLSv1_2) 
		<< "TLS version should be CURL_SSLVERSION_MAX_TLSv1_2 (393216) when explicitly set";
	
	// Verify config owner is DEV_CFG
	EXPECT_EQ(mAampConfig->GetConfigOwner(eAAMPConfig_TLSVersion), AAMP_DEV_CFG_SETTING);
}

/**
 * @brief Test eAAMPConfig_TLSVersion with aamp.cfg file
 * 
 * Verifies that TLS version can be configured via aamp.cfg file using the 'supportTLS' parameter.
 * Tests both default value and CURL_SSLVERSION_MAX_TLSv1_2 (393216).
 */
TEST_F(AampConfigTests, TLSVersionFromConfigFile)
{
	AampConfig aampConfig;
	aampConfig.Initialize();
	
	// Test default value first
	int defaultTlsVersion = aampConfig.GetConfigValue(eAAMPConfig_TLSVersion);
	EXPECT_EQ(defaultTlsVersion, 6) << "Default TLS version from config should be 6";
	...
	EXPECT_EQ(tlsVersion, 393216) 
		<< "TLS version should be 393216 after loading from config file";
}

/**
 * @brief Test eAAMPConfig_TLSVersion with curl verbose logging enabled
 * 
 * Verifies that TLS version configuration works correctly when curl verbose logging
 * is enabled (curl=true). This allows verification of TLS version negotiation in logs.
 */
TEST_F(AampConfigTests, TLSVersionWithCurlLogging)
{
	mAampConfig->Initialize();
	...
	// Test default TLS version with logging enabled
	int defaultTlsVersion = mAampConfig->GetConfigValue(eAAMPConfig_TLSVersion);
	EXPECT_EQ(defaultTlsVersion, 6) 
		<< "Default TLS version should be 6 even with curl logging enabled";
	...
	int tlsVersion = mAampConfig->GetConfigValue(eAAMPConfig_TLSVersion);
	EXPECT_EQ(tlsVersion, CURL_SSLVERSION_MAX_TLSv1_2) 
		<< "TLS version should be 393216 with curl logging enabled";
}

/**
 * @brief Test eAAMPConfig_TLSVersion range validation
 * 
 * Verifies that TLS version values are validated against the valid range
 * (CURL_SSLVERSION_DEFAULT to CURL_SSLVERSION_MAX_LAST).
 */
TEST_F(AampConfigTests, TLSVersionRangeValidation)
{
	mAampConfig->Initialize();
...
/**
 * @brief Test eAAMPConfig_TLSVersion for both media segments and manifests
 * 
 * Verifies that the same TLS version configuration applies to both media segments
 * and manifest downloads, ensuring consistent TLS negotiation across all HTTPS downloads.
 */
TEST_F(AampConfigTests, TLSVersionAppliesGlobally)
{
	mAampConfig->Initialize();
	
	// The TLS version setting should apply globally to all HTTPS connections
	// (both media segments and manifests use the same configuration)
	const int CURL_SSLVERSION_TLSv1_2 = 6;
	
	int tlsVersion = mAampConfig->GetConfigValue(eAAMPConfig_TLSVersion);
	EXPECT_EQ(tlsVersion, CURL_SSLVERSION_TLSv1_2) 
		<< "TLS version configuration should apply to all downloads (media and manifests)";
	...
	EXPECT_EQ(tlsVersion, CURL_SSLVERSION_MAX_TLSv1_2) 
		<< "Updated TLS version should apply globally";
