...
TEST_F(StreamAbstractionAAMP_HLSTest, GetStreamFormat_UseMp4DemuxAndClearHlsMp4_ReturnsCodecFormats)
{
	if (mStreamAbstractionAAMP_HLS->trackState[eTRACK_VIDEO] == nullptr)
	{
		mStreamAbstractionAAMP_HLS->trackState[eTRACK_VIDEO] =
	...
	streamInfo.codecs = "hvc1.2.4.L120.90,ec-3";
	mStreamAbstractionAAMP_HLS->streamInfoStore.push_back(streamInfo);
	mStreamAbstractionAAMP_HLS->SetHasDrm(false);
	mPrivateInstanceAAMP->mMediaFormat = eMEDIAFORMAT_HLS_MP4;
	...
	EXPECT_EQ(primaryOutputFormat, FORMAT_VIDEO_ES_HEVC);
	EXPECT_EQ(audioOutputFormat, FORMAT_AUDIO_ES_EC3);
	EXPECT_EQ(subtitleOutputFormat, FORMAT_SUBTITLE_WEBVTT);
}

TEST_F(StreamAbstractionAAMP_HLSTest, GetStreamFormat_UseMp4DemuxAndEncryptedHlsMp4_ReturnsCodecFormats)
{
	if (mStreamAbstractionAAMP_HLS->trackState[eTRACK_VIDEO] == nullptr)
	{
		mStreamAbstractionAAMP_HLS->trackState[eTRACK_VIDEO] =
	...
	HlsStreamInfo streamInfo;
	streamInfo.codecs = "avc1.42c00d,mp4a.40.2";
	mStreamAbstractionAAMP_HLS->streamInfoStore.push_back(streamInfo);
	mStreamAbstractionAAMP_HLS->SetHasDrm(true);
	mPrivateInstanceAAMP->mMediaFormat = eMEDIAFORMAT_HLS_MP4;
	mStreamAbstractionAAMP_HLS->trackState[eTRACK_VIDEO]->streamOutputFormat = FORMAT_ISO_BMFF;
	mStreamAbstractionAAMP_HLS->trackState[eTRACK_AUDIO]->streamOutputFormat = FORMAT_ISO_BMFF;
	mStreamAbstractionAAMP_HLS->trackState[eTRACK_SUBTITLE]->streamOutputFormat = FORMAT_SUBTITLE_WEBVTT;
...
TEST_P(ParseManifestTagTest, ParsesWithoutCrash)
{
	...
	mStreamAbstractionAAMP_HLS->mainManifest.assign(tag.begin(), tag.end());
