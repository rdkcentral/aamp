...
TEST_F(FunctionalTests, TSBWriteTests)
{
	...
	cachedFragment->fragment.insert(cachedFragment->fragment.end(), reinterpret_cast<const uint8_t*>(TEST_DATA), reinterpret_cast<const uint8_t*>(TEST_DATA) + strlen(TEST_DATA));
	...
	// After std::vector refactoring, use fragment.data() which returns the internal buffer pointer
...
TEST_F(FunctionalTests, TSBReadTests)
{
	...
	cachedFragment->fragment.insert(cachedFragment->fragment.end(), reinterpret_cast<const uint8_t*>(TEST_DATA), reinterpret_cast<const uint8_t*>(TEST_DATA) + TEST_DATA_LEN);
