...
protected:
	...
	void SetUp() override
	{
		...
		g_mockTSBReader = std::make_shared<NiceMock<MockTSBReader>>();
...
TEST_F(AampTsbSessionManagerTests, TrackDisabled)
{
	...
	EXPECT_FALSE(mAampTSBSessionManager->PushNextTsbFragment(mMediaStreamContext.get()));
}
...
TEST_F(AampTsbSessionManagerTests, ReadInitFragmentFailure)
{
	...
	std::shared_ptr<TsbInitData> mockInitData = std::make_shared<TsbInitData>(
		"dummyInitUrl", eMEDIATYPE_VIDEO, StreamInfo{}, "dummyPeriodId", 0
	);
	...
	EXPECT_CALL(*g_mockTSBReader, ReadNext()).WillOnce(Return(mockFragmentData));
...
TEST_F(AampTsbSessionManagerTests, SameInitFragment)
{
	...
	auto mockInitData = std::make_shared<TsbInitData>("dummyInitUrl", eMEDIATYPE_VIDEO, StreamInfo{}, "dummyPeriodId", 0);
	// Create a TsbFragmentData object with the dummy parameters
	auto mockFragmentData{std::make_shared<TsbFragmentData>(
		dummyUrl, dummyMediaType, dummyPosition, dummyDuration, dummyPts, dummyDisc,
		dummyPrId, mockInitData, dummyTimeScale, dummyPTSOffsetSec
...
TEST_F(AampTsbSessionManagerTests, SkipFragments)
{
	...
	auto mockInitData = std::make_shared<TsbInitData>("dummyInitUrl", eMEDIATYPE_VIDEO, dummyStreamInfo, "dummyPeriodId", 0);
	...
	EXPECT_CALL(*g_mockTSBReader, ReadNext()).WillRepeatedly([=]() mutable {
	...
	EXPECT_CALL(*g_mockTSBReader, IsFirstDownload()).WillOnce(Return(false));
