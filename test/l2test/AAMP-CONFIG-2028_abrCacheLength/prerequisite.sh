...
download_stream() {
    ...
    if (curl -O  -X GET $1);then
        ...
        If the URL is incorrect, set the environmental variable TEST_2028_STREAM_PATH to the right value in .env file under l2test folder"
        return 1
    fi
    return 0
}
...
if [ "$TEST_2028_STREAM_PATH" == "" ]; then
    TEST_2028_STREAM_PATH="https://cpetestutility.stb.r53.xcal.tv/VideoTestStream/public/aamptest/testApps/L2/AAMP-IFRAME-4007/VideoTestStream.tar.xz"
fi

echo $(pwd)
try=1
download_stream "${TEST_2028_STREAM_PATH}"
while [[ "$?" != 0 && "${try}" < 3 ]]; do
    try=$((try + 1))
    download_stream "${TEST_2028_STREAM_PATH}"
done
