...
less than 1 second. It uses a pre-downloaded file so as not to include download time.
...
1. Streams for this test will be downloaded from https://cpetestutility.stb.r53.xcal.tv
   but location can be overridden by specifying TEST_1004_STREAM_PATH if required.

    a. When running inside docker: -

         echo "TEST_1004_STREAM_PATH=https://artifactory.host.com/artifactory/stream_data.gz" >> .env

    b. When running on ubuntu (outside a docker container): -

        export TEST_1004_STREAM_PATH=https://artifactory.host.com/artifactory/stream_data.gz
