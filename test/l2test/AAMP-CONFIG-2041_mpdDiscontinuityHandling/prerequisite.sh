...
download_stream() {
    ...
    if (curl -O  -X GET $1);then
        ...
        If the URL is incorrect, set the environmental variable TEST_2041_STREAM_PATH to the right value in .env file under l2test folder"
        return 1
    fi
    return 0
}
...
if [ "$TEST_2041_STREAM_PATH" == "" ]; then
    TEST_2041_STREAM_PATH="https://cpetestutility.stb.r53.xcal.tv/VideoTestStream/public/aamptest/streams/L2/discontinuityTestStream.tar.xz"
fi

echo $(pwd)
try=1
download_stream "${TEST_2041_STREAM_PATH}"
while [[ "$?" != 0 && "${try}" < 3 ]]; do
    try=$((try + 1))
    download_stream "${TEST_2041_STREAM_PATH}"
done
