...
TESTDATA1 = {
    ...
    "expect_list": [
        ...
        {"expect": r"aamp url:0,0,0,2.000000,http://localhost:8080/content/dash/*?(1080|720|480|360)p_0([1][6-9]|[2][0-9]|30).m4s", "max": 180, "not_expected": True},
        # Make sure there are no stall at discontinuity processing
        {"expect": r"\[CheckForMediaTrackInjectionStall\]", "max": 180, "not_expected": True},
        ...
        {"expect": r"\[AAMPCLI\] AAMP_EVENT_TIMED_METADATA place advert breakId\=1 adId\=adId1 url\=.*?ad_30s.mpd", "max": 50},
        ...
        {"expect": re.escape("Period ID changed from '0' to '0-111' [BasePeriodId='1']"), "min": 00, "max": 180},
...
TESTDATA2 = {
    ...
    "expect_list": [
        ...
        {"expect": r"aamp url:0,0,0,2.000000,http://localhost:8080/content/dash/*?(1080|720|480|360)p_0(3[1-5]).m4s", "max": 180, "not_expected": True},
        # Make sure there are no stall at discontinuity processing
        {"expect": r"\[CheckForMediaTrackInjectionStall\]", "max": 180, "not_expected": True},
        # Confirm that an ad break is detected in Period 1 with a duration of 30 seconds
        {"expect": r"\[FoundEventBreak\]\[\d+\]\[CDAI\] Found Adbreak on period\[1\] Duration\[30000\] isDAIEvent\[1\]", "max": 50},
        {"expect": r"\[AAMPCLI\] \[CDAI\] Dynamic ad start signalled", "max": 50},
        {"expect": r"\[AAMPCLI\] AAMP_EVENT_TIMED_METADATA place advert breakId\=1 adId\=adId1 url\=.*?ad_30s.mpd", "max": 50},
        {"expect": r"\[onAdEvent\]\[\d+\]\[CDAI\]: STARTING ADBREAK\[1\] AdIdx\[0\] Found at Period\[1\]", "min": 10, "max": 60},
        {"expect": r"\[onAdEvent\]\[\d+\]\[CDAI\]: State changed from \[OUTSIDE_ADBREAK\] \=\> \[IN_ADBREAK_AD_PLAYING\].", "min": 10, "max": 60},
        ...
        {"expect": r"\[AAMPCLI\] \[CDAI\] Dynamic ad start signalled", "max": 120},
        {"expect": r"\[AAMPCLI\] AAMP_EVENT_TIMED_METADATA place advert breakId\=2 adId\=adId2 url\=.*?ad_10s.mpd", "min": 10, "max": 120},
        # Verify the state changes back to OUTSIDE_ADBREAK after completing the ad
        {"expect": r"\[PlaceAds\]\[\d+\]\[CDAI\] Placement Done: \{AdbreakId: 1, duration: 30000, endPeriodId: 2, endPeriodOffset: 0, \#Ads: 1", "min": 30, "max": 120},
        {"expect": r"\[onAdEvent\]\[\d+\]\[CDAI\]: State changed from \[IN_ADBREAK_WAIT2CATCHUP\] \=\> \[OUTSIDE_ADBREAK\].", "min": 30, "max": 120},
...
TESTDATA3 = {
    ...
    "expect_list": [
        ...
        {"expect": r"HttpRequestEnd: 0,1,404.*?http://localhost:8080/content/dash/en_0(04|07|10).mp3\?live=true", "max": 50},
        # Ensure the position jump is handled correctly
