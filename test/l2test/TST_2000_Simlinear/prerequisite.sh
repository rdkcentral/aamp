...
if (curl -O  -X GET $TEST_2000_STREAM_PATH);then
    ...
    echo "Downloading the stream failed. Check that the URL '$TEST_2000_STREAM_PATH' can be reached.
    If the URL is incorrect, set the environmental variable TEST_2000_STREAM_PATH to the right value in .env file under l2test folder"
fi
