...
if [ "$TEST_2047_STREAM_PATH" == "" ]; then
    TEST_2047_STREAM_PATH="https://cpetestutility.stb.r53.xcal.tv/VideoTestStream/public/aamptest/streams/sky/skywitness-4klive-8M.tar.gz"
fi

echo $(pwd)
try=1
download_stream "${TEST_2047_STREAM_PATH}"
while [[ "$?" != 0 && "${try}" < 3 ]]; do
    try=$((try + 1))
    download_stream "${TEST_2047_STREAM_PATH}"
done
