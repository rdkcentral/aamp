...
Test used to check/verify the linear CDAI behaviour with Single source period with CDAI ad long by <= 2 sec
...
1. Streams for this test will be downloaded from https://cpetestutility.stb.r53.xcal.tv
   but location can be overridden by specifying TEST_8007_STREAM_PATH if required.

    a. When running inside docker: -

         echo "TEST_8007_STREAM_PATH=https://artifactory.host.com/artifactory/stream_data.gz" >> .env

    b. When running on ubuntu (outside a docker container): -

        export TEST_8007_STREAM_PATH=https://artifactory.host.com/artifactory/stream_data.gz
