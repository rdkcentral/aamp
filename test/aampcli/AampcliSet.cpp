				...
				case 40:
					{
						...
						if (sscanf(cmd, "set %s %d", command, &id3MetadataEventsEnabled) == 2){
							...
							{
								playerInstanceAamp->AddEventListener(AAMP_EVENT_ID3_METADATA, std::static_pointer_cast<EventListener>(lAampcli.mEventListener));
							}
							else
							{
								playerInstanceAamp->RemoveEventListener(AAMP_EVENT_ID3_METADATA, std::static_pointer_cast<EventListener>(lAampcli.mEventListener));
							}
				...
				case 53:
					{
						...
						if (sscanf(cmd, "set %s %d", command, &mediaMetadataEventsEnabled) == 2){
							...
							{
								playerInstanceAamp->AddEventListener(AAMP_EVENT_MEDIA_METADATA, std::static_pointer_cast<EventListener>(lAampcli.mEventListener));
							}
							else
							{
								playerInstanceAamp->RemoveEventListener(AAMP_EVENT_MEDIA_METADATA, std::static_pointer_cast<EventListener>(lAampcli.mEventListener));
							}
						}
						else
						{
				...
				case 55:
					{
						...
						if (sscanf(cmd, "set %s %d", command, &timeout) == 2)
						{
							...
							playerInstanceAamp->AddEventListener(AAMP_EVENT_CONTENT_PROTECTION_DATA_UPDATE, std::static_pointer_cast<EventListener>(lAampcli.mEventListener));
