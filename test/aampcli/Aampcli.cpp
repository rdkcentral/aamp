		...
		case AAMP_EVENT_PROGRESS:
			{
				...
				if(mAampcli.mEnableProgressLog)
				{
					...
					AAMPCLI_PRINTF("[AAMPCLI] AAMP_EVENT_PROGRESS duration=%.3fs position=%.3fs seekableRange%s currRate=%.3f bufferedVideoDuration=%.3fs bufferedAudioDuration=%.3fs  PTS=%lld timecode='%s' latency=%.3fs profileBandwidth=%" BITSPERSECOND_FORMAT " networkBandwidth=%" BITSPERSECOND_FORMAT " currentPlayRate=%.3f sessionId='%s' targetLatency=%.3fs\n", ev->getDuration()/1000.0, ev->getPosition()/1000.0, seekableRange, ev->getSpeed(), ev->getVideoBufferedDuration()/1000.0, ev->getAudioBufferedDuration()/1000.0, ev->getPTS(), ev->getSEITimeCode(), ev->getLiveLatency()/1000.0, ev->getProfileBandwidth(), ev->getNetworkBandwidth(), ev->getCurrentPlayRate(), ev->GetSessionId().c_str(), ev->getTargetLatency()/1000.0);
				}
			}
			break;
