		...
		if (aamp->mpStreamAbstractionAAMP && !(aamp->mbUsingExternalPlayer))
		{
			...
			if(!(aamp->mbPlayEnabled) && aamp->mSinkPaused.load() && (AAMP_RATE_PAUSE != rate) && (aamp->mbSeeked || !aamp->mbDetached))
			{
				...
				if (AAMP_NORMAL_PLAY_RATE == rate)
				{
					...
					if (sink)
					{
						sink->Configure(StreamCodecInfo{aamp->GetMediaCodecInfo(eMEDIATYPE_VIDEO), aamp->GetMediaCodecInfo(eMEDIATYPE_AUDIO), aamp->GetMediaCodecInfo(eMEDIATYPE_SUBTITLE)},
