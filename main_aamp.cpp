			...
			{ // no change in desired play rate
				...
				if (aamp->pipeline_paused && rate != 0)
				{
					...
					if (aamp->playerStartedWithTrickPlay)
					{
						...
						if (sinkForTrickplay)
						{
							sinkForTrickplay->Configure(aamp->mVideoFormat, aamp->mAudioFormat, aamp->mAuxFormat, aamp->mSubtitleFormat,
								aamp->mpStreamAbstractionAAMP->GetESChangeStatus(), aamp->mpStreamAbstractionAAMP->GetAudioFwdToAuxStatus());
						}
						aamp->mpStreamAbstractionAAMP->StartInjection();
					}
