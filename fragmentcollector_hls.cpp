...
AAMPStatusType StreamAbstractionAAMP_HLS::ParseMainManifest()
{
	...
	lstring iter = lstring(reinterpret_cast<char*>(mainManifest.data()), mainManifest.size());
	while( !iter.empty() )
	{
...
bool TrackState::FetchFragmentHelper(int &http_error, bool &decryption_error, bool & bKeyChanged, int * fogError, double &downloadTime)
{
		...
		if (!mInjectInitFragment && !fragmentURI.empty() && !bSegmentRepeated)
		{
			...
			cachedFragment->fragment.insert(cachedFragment->fragment.end(), webvtt, webvtt + 6);
			}
			if (!fetched)
			{
...
void TrackState::IndexPlaylist(bool IsRefresh, AampTime &culledSec)
{
	...
	lstring iter = lstring(reinterpret_cast<char*>(playlist.data()), playlist.size());
	if( !iter.empty() ){
		lstring ptr = iter.mystrpbrk();
		if( !ptr.equal("#EXTM3U") )
		{
...
TrackState::TrackState(TrackType type, StreamAbstractionAAMP_HLS* parent, PrivateInstanceAAMP* aamp, const char* name,
		...
		mSyncAfterDiscontinuityInProgress(false), playlist(),
...
TrackState::~TrackState()
{
	...
	{
		mCachedFragment[j].fragment.clear();
		mCachedFragment[j].fragment.shrink_to_fit();
	}
	FlushIndex();
...
StreamAbstractionAAMP_HLS::~StreamAbstractionAAMP_HLS()
{
	...
	aamp->SyncBegin();
	this->thumbnailManifest.clear();
		thumbnailManifest.shrink_to_fit();
	this->mainManifest.clear();
		mainManifest.shrink_to_fit();
		...
		if( streamInfo.isIframeTrack && isThumbnailStream(streamInfo) )
		{
			...
			else
			{
				...
				if( aamp->GetFile(std::move(url), eMEDIATYPE_PLAYLIST_IFRAME, &thumbnailManifest, tempEffectiveUrl, &http_error, &tempDownloadTime, NULL, eCURLINSTANCE_MANIFEST_MAIN,true) )
				{
					...
					{
						lstring iter = lstring(reinterpret_cast<char*>(thumbnailManifest.data()), thumbnailManifest.size());
...
void StreamAbstractionAAMP_HLS::HandleSleThumbnailData(double tStart, double tEnd)
{
	...
	lstring thumbNailIter = lstring(reinterpret_cast<char*>(thumbnailManifest.data()), thumbnailManifest.size());
...
std::vector<ThumbnailData> StreamAbstractionAAMP_HLS::GetThumbnailRangeData(double tStart, double tEnd, std::string *baseurl, int *raw_w, int *raw_h, int *width, int *height)
{
	...
	ContentType type = aamp->GetContentType();
	if(!thumbnailManifest.data() || ( type == ContentType_SLE || type == ContentType_LINEAR ) )
	{
		thumbnailManifest.clear();
		thumbnailManifest.shrink_to_fit();
		std::string tmpurl;
...
void TrackState::UpdateDrmCMSha1Hash( const std::string &newSha1Hash )
{
	...
	if(drmDataChanged)
	{
		...
		if( i == mDrmMetaDataIndex.size() )
		{
			...
			// use printf to avoid 2048 char syslog limitation
			printf("***playlist***:\n\n%.*s\n************\n", (int)playlist.size(), playlist.data());
			assert(false);
		}
	}
}
