										 ...
										 uint32_t payloadDataSize, uint8_t **ppOpaqueData)
{
	...
	MW_LOG_TRACE("DEBUG--> Before decrypt API 3");
	std::lock_guard<std::mutex> guard(decryptMutex);
	MW_LOG_TRACE("DEBUG--> After decryptMutex mutex 3");
	...
	MW_LOG_TRACE("DEBUG--> Before opencdm_session_decrypt call");
	...
	MW_LOG_TRACE("DEBUG--> After opencdm_session_decrypt call");
	if (retvalue != 0)
	{
		if (m_drmHelper->getMemorySystem() != nullptr)
		{
	...
	MW_LOG_TRACE("DEBUG--> After decrypt API 3 ret:%d", retvalue);
	return retvalue;
}
