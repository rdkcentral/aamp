...
void AmlogicSocInterface::SetSeamlessSwitch(GstElement* sink, gboolean value)
{
    ...
    if (GST_IS_ELEMENT(sink)) {
        MW_LOG_WARN("VRN AMLOGIC:SetSeamlessSwitch() sink is a valid GstElement");
    }
	else
	{
		MW_LOG_WARN("VRN AMLOGIC:SetSeamlessSwitch() sink is not a valid GstElement, cannot set property");
	}
