...
#ifndef PLAYER_SOC_INTERFACE_IMPL_H
#define PLAYER_SOC_INTERFACE_IMPL_H

#include "SocInterface.h"


class PlayerSocInterfaceImpl : public SocInterface
{
...
public:
	 ...
	 *
	 * This function returns the platform type of the current SoC.
	 *
	 * @return The platform type as a SocPlatformType enumeration value.
	 */
	virtual SocPlatformType GetPlatformType() override;

	/**
	 * @brief Get SVP Context
	 */
	virtual void SvpGetContext(void **svpCtx, int flags) override;

	/**
	 * @brief Free SVP Context
	 */
	virtual void SvpFreeContext(void *svpCtx) override;
	
	/*@brief returns true if video stats required from sink otherwise false*/
	virtual bool IsPlaybackQualityFromSink() override;
	 ...
	 */
	virtual void SetVideoBufferSize(GstElement *sink, int size)override;
	 ...
	 */
	virtual void SetSinkAsync(GstElement *sink, gboolean status)override;
	 ...
	 */
	virtual bool UseAppSrc()override;
	 ...
	 */
	virtual bool UseWesterosSink()override;
	 ...
	 */
	virtual bool IsAudioFragmentSyncSupported()override;
	 ...
	 */
	virtual bool EnableLiveLatencyCorrection()override;
	 ...
	 */
	virtual int RequiredQueuedFrames()override;
	 ...
	 */
	virtual bool EnablePTSRestamp()override;
	 ...
	 */
	virtual bool IsFirstTuneWithWesteros()override;
	
	/**
	 * @brief Set SoC volume property name.
	 */
	virtual void SetAudioProperty(const char * &volume, const char * &mute, bool& isSinkBinVolume)override;
	 ...
	 */
	virtual void SetSeamlessSwitch(GstElement* object,  gboolean value)override;
	 ...
	 */
	virtual bool AudioOnlyMode(GstElement *sinkbin)override;
	 ...
	 */
	virtual bool SetPlaybackRate(const std::vector<GstElement*>& sources, GstElement *pipeline, double rate, GstElement *video_dec, GstElement *audio_dec)override;
	 ...
	 */
	virtual GstPad* GetSourcePad(GstElement* element)override;
	
	/**
	 * @brief Get video sink from sinkbin.
	 * @param sinkbin The GStreamer sinkbin.
	 */
	virtual GstElement* GetVideoSink(GstElement* sinkbin)override;
	 ...
	 */
	virtual void SetAC4Tracks(GstElement *src, int trackId)override;
	
	/**
	 * @brief Set platform playback rate.
	 * @return True on success, false otherwise.
	 */
	virtual bool SetPlatformPlaybackRate()override;
	
	/**
	 * @brief Set rate correction.
	 * @return True on success, false otherwise.
	 */
	virtual bool SetRateCorrection()override;
	 ...
	 */
	virtual bool IsVideoSink(const char* name)override;
	 ...
	 */
	virtual bool IsAudioSinkOrAudioDecoder(const char* name)override;
	 ...
	 */
	virtual bool IsVideoDecoder(const char* name)override;
	 ...
	 * @return True on success, false otherwise.
	 */
	virtual bool ConfigureAudioSink(GstElement **audio_sink, GstObject *src, bool decStreamSync)override;
	 ...
	 */
	virtual bool IsAudioOrVideoDecoder(const char* name) override;
	 ...
	 */
	virtual bool DisableAsyncAudio(GstElement *audio_sink, int rate, bool isSeeking)override;
	 ...
	 * @param video_dec The video decoder element.
	 */
	virtual void GetCCDecoderHandle(gpointer *dec_handle, GstElement *video_dec)override;
	 ...
	 */
	virtual bool ResetTrickUTC(){override;
	 ...
	 */
	virtual long long GetVideoPts(GstElement *video_sink, GstElement *video_dec, bool isWesteros)override;
	
	/**
	 * @brief Notify first video frame.
	 */
	virtual bool NotifyVideoFirstFrame()override;
	 ...
	 */
	virtual void SetDecodeError(GstObject* src)override;
	 ...
	 * @param src The source object.
	 */
	virtual void SetFreerunThreshold(GstObject* src)override;
	 ...
	 */
	virtual bool RequiredElementSetup()override;
	...
	virtual bool HasFirstAudioFrameCallback()override;
	 ...
	 */
	virtual bool IsVideoSinkHandleErrors()override;
	 ...
	 */
	virtual void SetPlaybackFlags(gint &flags,  bool isSub)override;
	
	/**
	 * @brief checks if the firstFrame is received from the simulator
	 */
	virtual bool IsSimulatorFirstFrame()override;
	
	/**
	 * @brief checks if the sink is from the simulator
	 */
	virtual bool IsSimulatorSink()override;
	
	/**
	 * @brief Configure the plugin priority for PulseAudio.
	 */
	virtual void ConfigurePluginPriority()override;
	
	/**
	 * @brief checks if the teardown is required for simulator
	 */
	virtual bool ShouldTearDownForTrickplay()override;
	
	/**
	 * @brief checks if the video sample is from the simulator
	 */
	virtual bool IsSimulatorVideoSample()override;
	
	/**
	 *@brief Sets the platform specific H264 caps
	 */
	virtual void SetH264Caps(GstCaps *caps)override;
	
	/**
	 *@brief Sets the HEVC caps for simulator
	 */
	virtual void SetHevcCaps(GstCaps *caps)override;
	 ...
	 */
	virtual bool ResetNewSegmentEvent()override;
	 ...
	 */
	virtual bool IsVideoMaster(GstElement *videoSink)override;
};
#endif
