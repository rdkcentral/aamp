...
ContentProtectionFirebolt::ContentProtectionFirebolt() : mInitialized(false), mIsConnected(false), mSpeedStateMutex(), mCPFMutex(), mFMutex()
{
	Initialize();	
}
...
void ContentProtectionFirebolt::SubscribeEvents()
{
	...
	Firebolt::Error err = Firebolt::Error::None;
	...
	{
		MW_LOG_ERR("Failed to subscribe to watermark events: %d", static_cast<int>(err));
	}
}

void ContentProtectionFirebolt::UnSubscribeEvents()
{
	MW_LOG_INFO("Unsubscribing from Firebolt Content Protection events");
	Firebolt::Error err = Firebolt::Error::None;
	if (mEventHandler)
	{
		...
		if (err != Firebolt::Error::None)
		{
			MW_LOG_ERR("Failed to unsubscribe from watermark events: %d", static_cast<int>(err));
		}
		mEventHandler.reset();
	}
}
...
//if (errorInitialize == Firebolt::Error::None && errorConnect == Firebolt::Error::None)
	...
	//MW_LOG_ERR("Failed to create FireboltInstance InitializeError:\"%d\" ConnectError:\"%d\"", static_cast<int>(errorInitialize), static_cast<int>(errorConnect));
	return false;
}
...
void ContentProtectionFirebolt::CloseDrmSession(int64_t sessionId)
{
	...
	Firebolt::IFireboltAccessor::Instance().ContentProtectionInterface().closeDrmSession(std::to_string(sessionId), &error);
	// Check for errors
	...
	else
	{
		...
		MW_LOG_ERR("CloseDrmSession: failed for sessionID: %" PRId64 " Firebolt Error: \"%d\"", sessionId, static_cast<int>(error));
	}
}
bool ContentProtectionFirebolt::SetDrmSessionState(int64_t sessionId, bool active)
{
	...
	Firebolt::IFireboltAccessor::Instance().ContentProtectionInterface().setDrmSessionState(std::to_string(sessionId), sessionState, &error);
	// Check for errors
	if (error == Firebolt::Error::None)
	{
	...
	{
		// An error occurred, log the error
		MW_LOG_ERR("DRM session state failed to set to %d for sessionId: %" PRId64 ", Firebolt Error: \"%d\"", static_cast<int>(sessionState), sessionId, static_cast<int>(error));
	}
	return result;
}
/**
...
bool ContentProtectionFirebolt::SetPlaybackPosition(int64_t sessionId, float speed, int32_t position)
{
	...
	Firebolt::IFireboltAccessor::Instance().ContentProtectionInterface().setPlaybackPosition(std::to_string(sessionId), speed, position, &error);
	// Check for errors
	if (error == Firebolt::Error::None)
	{
	...
	{
		// An error occurred, log the error
		MW_LOG_ERR("SetPlaybackPosition failed to set for ID: %" PRId64 " Firebolt Error: \"%d\"", sessionId, static_cast<int>(error));
	}
	return ret;
}
/**
...
void ContentProtectionFirebolt::ShowWatermark(bool show, int64_t sessionId)
{
	...
	Firebolt::IFireboltAccessor::Instance().ContentProtectionInterface().showWatermark(std::to_string(sessionId), show, 0, &error);
	// Check for errors
	if (error == Firebolt::Error::None) {
		...
		// An error occurred, log the error
		MW_LOG_ERR("showWatermark failed. Firebolt Error: \"%d\"", static_cast<int>(error));
	}
}
...
bool ContentProtectionFirebolt::OpenDrmSession(std::string& clientId, std::string appId, std::string keySystem, std::string licenseRequest, std::string initData, int64_t sessionId, std::string &response)
{
	...
	if (error != Firebolt::Error::None)
	{
		MW_LOG_ERR("openDrmSession: Firebolt Error: \"%d\"", static_cast<int>(error));
	}
	else
	{
...
bool ContentProtectionFirebolt::UpdateDrmSession(int64_t sessionId, std::string licenseRequest, std::string initData, std::string &response)
{
	...
	std::string drmSession =  Firebolt::IFireboltAccessor::Instance()
		.ContentProtectionInterface()
		.updateDrmSession(std::to_string(sessionId),
				licenseRequest, initData, &error);
	if (error != Firebolt::Error::None)
	{
		MW_LOG_ERR("updateDrmSession: Firebolt Error: \"%d\"", static_cast<int>(error));
	}
	else
	{
		MW_LOG_INFO("DRM session updated successfully for sessionId: %" PRId64 " with Response %s", sessionId, drmSession.c_str());
		response = drmSession;
		ret = true;
	}
	return ret;
