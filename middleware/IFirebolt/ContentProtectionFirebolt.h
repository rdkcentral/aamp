 ...
 */
class ContentProtectionFirebolt : public ContentProtectionBase
{
public:
	/**
	 ...
	 * @brief Destructor
	 */
	~ContentProtectionFirebolt() override;
	 ...
	 */ 
	bool SetDrmSessionState(const std::string& sessionId, bool active) override;
	/**
	 * @brief Closes an existing DRM session
	 * @param sessionId DRM session ID
	 * @return true if closed successfully
	 */
	bool CloseDrmSession(const std::string& sessionId) override;
	/**
	 ...
	 */	
	bool OpenDrmSession(std::string& clientId, std::string appId, std::string keySystem, std::string licenseRequest, std::string initData, std::string& sessionId, std::string &response) override;
	/**
	 ...
	 * @return true on success
	 */	
	bool UpdateDrmSession(const std::string& sessionId, std::string licenseRequest, std::string initData, std::string &response) override;
	/**
	 ...
	 */	
	bool SetPlaybackPosition(const std::string& sessionId, float speed, int32_t position) override;
	/**
	 * @brief Enables or disables visual watermark
	 * @param show Show/hide flag
	 * @param sessionId Session context (optional)
	 */
	void ShowWatermark(bool show, const std::string& sessionId="");
...
private:
	...
	bool mIsConnected;
	/**
	 * @brief Returns current init state of the Firebolt client
	 * @return true if fully initialized
	 */
	bool Initialized();
	std::mutex mCallbackMutex;

	std::mutex mFireboltMutex;
	std::mutex mCPFMutex;
	std::mutex mSpeedStateMutex;
	bool mInitialized;
	std::unique_ptr<WatermarkEventHandler> mEventHandler;
};
