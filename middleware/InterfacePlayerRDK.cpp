	...
	if (eGST_MEDIATYPE_SUBTITLE == streamId)
	{
		if(m_gstConfigParam->gstreamerSubsEnabled)
		{
			if (interfacePlayerPriv->gstPrivateContext->usingRialtoSink)
			{
				...
				MW_LOG_INFO("subs using rialto subtitle sink muted=%d", interfacePlayerPriv->gstPrivateContext->subtitleMuted);
			...
			{
				MW_LOG_INFO("subs using subtecbin muted=%d",interfacePlayerPriv->gstPrivateContext->subtitleMuted);
...
void InterfacePlayerRDK::SetSubtitleMute(bool muted)
{
	...
	if (interfacePlayerPriv->gstPrivateContext->subtitle_sink)
	{
		...
		MW_LOG_INFO("subtitle_sink is NULL muted %d", muted);
}
