...
mProfilerCallback(nullptr), mProfilerBeginCallback(nullptr), mProfilerEndCallback(nullptr)
{
										   ...
										   bool isSubEnable, int32_t trackId, gint rate, const char *pipelineName, int PipelinePriority, bool FirstFrameFlag, std::string manifestUrl)
{
	...
	MW_LOG_MIL("[GSTPROFILING]ConfigurePpeline");
	...
	{		
		MW_LOG_MIL("Setting state to GST_STATE_PAUSED - pause on playback enabled");
		MW_LOG_MIL("[GSTPROFILING] Pipeline set to PAUSED");
	...
	{
		MW_LOG_MIL("Setting state to GST_STATE_PLAYING");
...
int InterfacePlayerRDK::SetupStream(int streamId,  void *playerInstance, std::string manifest)
{
	...
	else
	{
		...
		{
			MW_LOG_MIL("using tcpserversink");
			...
			if (eGST_MEDIATYPE_VIDEO == streamId)
			{
				ProfileBeginBucket(PROFILE_BUCKET_GST_VIDEO_SINK_ADD);

				g_object_set (G_OBJECT (sink), "port", tcp_port,"host","127.0.0.1",NULL);
				g_object_set(stream->sinkbin, "video-sink", sink, NULL);

				ProfileEndBucket(PROFILE_BUCKET_GST_VIDEO_SINK_ADD);

			}
			else if (eGST_MEDIATYPE_AUDIO == streamId)
			{
				ProfileBeginBucket(PROFILE_BUCKET_GST_AUDIO_SINK_ADD);
				...
				ProfileEndBucket(PROFILE_BUCKET_GST_AUDIO_SINK_ADD);

			}
		}
		else if (interfacePlayerPriv->gstPrivateContext->usingRialtoSink && eGST_MEDIATYPE_VIDEO == streamId)
		{
			MW_LOG_MIL("using rialtomsevideosink");
			GstElement* vidsink = gst_element_factory_make("rialtomsevideosink", NULL);
			if (vidsink)
			{
				MW_LOG_MIL("Created rialtomsevideosink: %s", GST_ELEMENT_NAME(vidsink));
				
				// Profile Rialto video sink creation
				pInterfacePlayerRDK->ProfileBeginBucket(PROFILE_BUCKET_RIALTO_VIDEO_SINK_CREATED);

				g_object_set(stream->sinkbin, "video-sink", vidsink, NULL);				/* In the stream->sinkbin, set the video-sink property to vidsink */
				MW_LOG_MIL("[GSTPROFILING] rialtomsevideosink element added to the pipeline");
 				pInterfacePlayerRDK->timestamps.rialtomsevideosink_time = NOW_STEADY_TS_MS;
				
				// Profile Rialto video sink creation
				pInterfacePlayerRDK->ProfileEndBucket(PROFILE_BUCKET_RIALTO_VIDEO_SINK_CREATED);
		...
		{
			MW_LOG_MIL("using rialtomseaudiosink");
			GstElement* audSink = gst_element_factory_make("rialtomseaudiosink",NULL);
			if(audSink)
			{
				MW_LOG_MIL("Created rialtomseaudiosink : %s",GST_ELEMENT_NAME(audSink));
				pInterfacePlayerRDK->ProfileBeginBucket(PROFILE_BUCKET_RIALTO_AUDIO_SINK_CREATED);
				...
				pInterfacePlayerRDK->ProfileEndBucket(PROFILE_BUCKET_RIALTO_AUDIO_SINK_CREATED);
	...
	MW_LOG_MIL("SinkBin add to pipeline");
	pInterfacePlayerRDK->ProfileBeginBucket(PROFILE_BUCKET_SINKBIN_ADD_TO_PIPELINE);

	gst_bin_add(GST_BIN(interfacePlayerPriv->gstPrivateContext->pipeline), stream->sinkbin);	
	
	pInterfacePlayerRDK->ProfileEndBucket(PROFILE_BUCKET_SINKBIN_ADD_TO_PIPELINE);
/* Add the stream sink to the pipeline */
	...
	{
		MW_LOG_MIL("element setup callback for demux");
		pInterfacePlayerRDK->ProfileBeginBucket(PROFILE_BUCKET_ELEMENT_SETUP_CB_ADD);
		...
		pInterfacePlayerRDK->ProfileEndBucket(PROFILE_BUCKET_ELEMENT_SETUP_CB_ADD);
...
void InterfacePlayerRDK::NotifyFirstFrame(int mediaType)
{
	...
	if (eGST_MEDIATYPE_VIDEO == mediaType)
	{
		ProfileBeginBucket(PROFILE_BUCKET_GST_VIDEO_DECODER_FIRST);

		MW_LOG_MIL("OnFirstVideoFrame. got First Video Frame");

		// Profile video decoder first frame
		ProfileEndBucket(PROFILE_BUCKET_GST_VIDEO_DECODER_FIRST);
		ProfileRialtoEvent(PROFILE_BUCKET_DECRYPTION_END);
	...
	{
		ProfileBeginBucket(PROFILE_BUCKET_GST_AUDIO_DECODER_FIRST);

		MW_LOG_MIL("OnFirstAudioFrame. got First Audio Frame");

		// Profile audio decoder first frame
		ProfileEndBucket(PROFILE_BUCKET_GST_AUDIO_DECODER_FIRST);
	...
	switch(GST_MESSAGE_TYPE(msg))
	{
		case GST_MESSAGE_STATE_CHANGED:
			...
			if (GST_MESSAGE_SRC(msg) == GST_OBJECT(privatePlayer->gstPrivateContext->pipeline))
			{
				...
				{
					pInterfacePlayerRDK->ProfileBeginBucket(PROFILE_BUCKET_GST_PIPELINE_PLAYING);
