...
| miniWindowAudioOnly | Boolean | false | Enable audio-only playback when the IP video rectangle enters mini-window dimensions. |
...
| liveOffset | Number | 15 | Target delay (seconds) from the live edge for regular live streams. Player starts and aims to stay about this far behind live. |
| liveOffset4K | Number | 15 | Target delay (seconds) from the live edge for 4K live streams. Same behavior as liveOffset, but applied to 4K playback. |
| cdvrLiveOffset | Number | 30 | Sets the delay (seconds) behind the live edge for CDVR playback. A larger value starts CDVR playback farther behind live. |
...
1. The DASH server makes each segment available in smaller parts using
    partial-segment delivery (for example, chunked transfer) with
    `SegmentTemplate@availabilityTimeOffset`.
2. AAMP enables LL-DASH only when the MPD indicates partial availability:
    `availabilityTimeOffset` is present and greater than `0`, and
    `availabilityTimeComplete` is `false`.
3. After LL-DASH is enabled, AAMP reads `ServiceDescription` values for
    `Latency` and `PlaybackRate` when present. If those values are missing,
    AAMP uses configured fallback values.
4. The player starts downloading and injecting media as chunks arrive,
    instead of waiting for the full segment to complete.
5. A latency-correction loop continuously compares current latency against
    the min/target/max window and adjusts playback speed within
    `minPlaybackRate` and `maxPlaybackRate`.
6. In LL-DASH mode, AAMP uses chunk/progress-level bandwidth samples for 
    faster ABR estimation, but it still applies bitrate profile switches
...
| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| lowLatencyMinValue | Number | 5 | If latency drops below this (seconds), playback slows to 0.97x until latency recovers to lowLatencyTargetValue. |
...
| lowLatencyMaxValue | Number | 7 | If latency exceeds this (seconds) and buffer is at or above lowLatencyTargetBuffer, playback speeds up to 1.03x until latency drops to lowLatencyTargetValue. |

#### Dynamic Latency Adjustment

Defines parameters used to automatically adjust target latency in response to changing network conditions and playback stability.

| Property | Type | Default | Description |
| -------- | ---- | ------- | ----------- |
| latencyRestorationBufferSec | Number | 2 | Minimum buffer level (seconds) that must be maintained throughout the latency restoration stability window before target latency is reduced by one step. If the buffer drops below this value, the restoration timer is reset. |
