				...
				if(openPrdFound && -1 != mPlacementObj.curAdIdx && (mPlacementObj.openPeriodId == periodId))
				{
					...
					if ((periodDelta == 0) && (nextPeriodDur > 0))
					{
						...
						if (nextPeriod)
						{
							...
							if( abObj.ads )
							{
								// Adjusting periodDurationAvailable only if ad's basePeriodId
								// matches current periodId.This avoids incorrect calculation in split period cases
								// where an ad spans into the next period which could otherwise result in a negative value
